...
| **TD-6** | **Circular-dependency gate** (Tarjan SCC over the import graph) | the layered seams are clean today; no cycle exists yet | a future import cycle silently couples layers (violates R-1) | a `gate:cycles` script baselines SCCs and fails on a new cycle | **P3** |
